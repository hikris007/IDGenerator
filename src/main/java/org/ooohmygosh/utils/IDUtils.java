...
public class IDUtils {
    ...
    public static char calcCheckDigit(String IDNumber) {
        if(IDNumber == null)
            throw new IllegalArgumentException("ID Number can't be null");

        if(IDNumber.length() < 17)
            throw new IllegalArgumentException("ID Number can't be less then 17 char");
        ...
        for(int i=0;i<17;i++){
            char c = chars[i];
            if(c < 48 || c > 57)
                throw new IllegalArgumentException("ID Number contains illegal characters");
    ...
    public static String formatter(ID id) throws Exception {
